...
Next we make a time series plot to visualize the average number of steps taken over the 5-minute intervals reported.  Again we call the aggregate function to isolate the steps and interval variables. 

The highest average number of steps (206.1698) is  at 835th interval 
...
We can fill in the missing values in the data set by first indexing the NA values and replacing them with the mean average of the values with the data set.  

Before we do this, we want to preserve the orginal data set for comparison sake, so we'll duplicate the data frame and assign the variable activity2, then we'll replace the NA values in that data frame with the following code:   
...
```
Here we see that the impact of replacing the NA values changes the frequency on the histogram, mostly for days calculated with 10k to 15k steps.  
